...
def create_series_dataloaders(
    X_train, eval_set, weights, batch_size, num_workers, drop_last, pin_memory, sequence_length
    ):
    need_shuffle, sampler = create_sampler(weights, X_train)
    ...
    for X in eval_set:
        if scipy.sparse.issparse(X):
            ...
            valid_dataloaders.append(
                DataLoader(
                    ShiftedTimeSeriesDataset(X,sequence_length),
                    batch_size=batch_size,
                    sampler=sampler,
                    shuffle=need_shuffle,
                    num_workers=num_workers,
