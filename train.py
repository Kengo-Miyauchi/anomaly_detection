...
def set_default_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--prompt_length", type=int, default=5)  # ← prompt のトークン長（例）
...
def train(args: argparse.Namespace):
    ...
    # Dataset / DataLoader
    train_ds = SCADADataset(train_pkl, prefix_length=args.prefix_length, prompt_length=args.prompt_length)  # ← prompt_length対応
    valid_ds = SCADADataset(valid_pkl, prefix_length=args.prefix_length, prompt_length=args.prompt_length)
    ...
    for epoch in range(args.epochs):
        ...
        for tokens, mask, prefix, prompt_tokens in pbar:  # prompt_tokens追加想定
            ...
            # logits : [B, P + prompt_length + T, V]
            # loss計算は prefix_length + prompt_length を除いて tokens部分のみ
        ...
        with torch.no_grad():
            for tokens, mask, prefix, prompt_tokens in pbar:
                tokens = tokens.to(device)
                mask = mask.to(device)
                prefix = prefix.to(device, dtype=torch.float32)
                prompt_tokens = prompt_tokens.to(device)
