...
def log_df_dz_exact(g, z):
    ...
    jac = [torch.autograd.grad(g[:, i].sum(), z, create_graph=True, retain_graph=True)[0] for i in range(n_dims)]
...
def log_df_dz_fixed(g, z, n_samples=1, n_power_series=8):
    ...
    w_t_J_fn = lambda w, z=z, g=g: torch.autograd.grad(g, z, grad_outputs=w, retain_graph=True, create_graph=True)[0]
    ...
    def _get_logdet_estimator(self):
        if self.training:
            ...
            if self.estimator == "exact":
                ...
                logdet_fn = lambda g, z: log_df_dz_unbias(g, z, n_samples=4, n_exact=8, is_training=self.training)
            else:
...
class InvertibleResLinear(InvertibleResBlockBase):
    def __init__(self, in_features, out_features, base_filters=32, n_layers=2, activation="lipswish", coeff=0.97, ftol=1.0e-4, logdet_estimator="unbias"):
...
class InvertibleResConv2d(InvertibleResBlockBase):
    def __init__(self, in_channels, out_channels, base_filters=32, n_layers=2, activation="lipswish", coeff=0.97, ftol=1.0e-4, logdet_estimator="unbias"):
