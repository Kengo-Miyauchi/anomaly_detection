# ref:https://github.com/YuanGongND/ssast/blob/main/src/models/ast_models.py
...
class ASTModel(nn.Module):
    def __init__(
        self, label_dim=527, fshape=128, tshape=2, fstride=128, tstride=2, input_fdim=128, input_tdim=1024, model_size="base", pretrain_stage=True, load_pretrained_mdl_path=None, id_header=False
    ):
        ...
        if pretrain_stage == True:
            ...
            self.cpredlayer = nn.Sequential(nn.Linear(self.original_embedding_dim, self.original_embedding_dim), nn.ReLU(), nn.Linear(self.original_embedding_dim, 64))
            # masked patch reconstruction (generative objective) layer
            self.gpredlayer = nn.Sequential(nn.Linear(self.original_embedding_dim, self.original_embedding_dim), nn.ReLU(), nn.Linear(self.original_embedding_dim, 64))
            ...
            # mlp head for ID Discrimination
            if id_header:
        ...
        # use a pretrained models for finetuning
            ...
            new_pos_embed = (
                self.v.pos_embed[:, self.cls_token_num :, :].detach().reshape(1, p_num_patches, self.original_embedding_dim).transpose(1, 2).reshape(1, self.original_embedding_dim, p_f_dim, p_t_dim)
            )
    ...
    def finetuningcls_feature(self, x):
        self.log_softmax = nn.LogSoftmax(dim=1)
        B = x.shape[0]
        x = self.v.patch_embed(x)
        if self.cls_token_num == 2:
    ...
    def mpc(self, x, mask_patch, cluster, show_mask=False):
        ...
        encode_samples = torch.empty((B, mask_patch, 64), device=x.device, requires_grad=False).float()
        ...
        pred = torch.empty((B, mask_patch, 64), device=x.device).float()  # e.g. size 12*100*768
        for i in range(B):
        ...
        if show_mask == False:
            ...
            for i in range(B):
                ...
                pred[i, mask_index[i], :] = torch.tensor(result).reshape(mask_patch, 1).expand(mask_patch, 64)
    ...
    def forward(self, x, task, cluster=True, mask_patch=400):
        ...
        elif task == "getfeature":
            return self.getfeature(x)
        elif task == "get_cls":
            return self.get_cls(x)
        elif task == "ft_cls_fe":
            return self.finetuningcls_feature(x)
        else:
...
if __name__ == "__main__":
    ...
    ast_mdl = ASTModel(fshape=16, tshape=16, fstride=16, tstride=16, input_fdim=128, input_tdim=input_tdim, model_size="base", pretrain_stage=True)
    ...
    ast_mdl = ASTModel(
        label_dim=35, fshape=16, tshape=16, fstride=10, tstride=10, input_fdim=128, input_tdim=input_tdim, model_size="base", pretrain_stage=False, load_pretrained_mdl_path="./test_mdl.pth"
    )
